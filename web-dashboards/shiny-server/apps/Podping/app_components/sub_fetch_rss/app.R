...
server <- function(input, output, session) {
  ...
  output$metadata <- renderText({
    ...
    feedTitle <-  unique(
      as.data.frame(
        feed
      )[c("meta.feed_title","meta.feed_description")]
    )
#    if (length(feedTitle[["meta.feed_link"]]) == 0 ) {
#      titleLink <-feedTitle[["meta.feed_title"]]
#    } else {
#      titleLink <-paste0("<a href='",feedTitle[["meta.feed_link"]],"'>",feedTitle[["meta.feed_title"]],"</a>")
#      titleLink <-feedTitle[["meta.feed_title"]]
#    }
    paste0(
      "<h2>",feedTitle["meta.feed_title"],"</h2>",
